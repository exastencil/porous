module Porous
  class RenderContext
    TAGS = %w(
      ...
      caption cite clipPath code col colgroup command datalist dd defs del details dfn div dl dt em embed fieldset
      figcaption figure footer form g h1 h2 h3 h4 h5 h6 head header hgroup hr html i iframe img input ins kbd keygen
      label legend li link map mark menu meta meter nav noscript object ol optgroup option output p param path pre
      progress q rect rp rt ruby s samp script section select small source span strong style sub summary sup svg table tbody
